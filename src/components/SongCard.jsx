    ...
    <div className="flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer">
      <div className="relative w-full h-56 group">
        <div className={`absolute inset-0 justify-center items-center bg-black bg-opacity-50 group-hover:flex ${
          activeSong?.title === song?.title && activeSong?.key === song?.key ? 'flex bg-black bg-opacity-70' : 'hidden'
        }`}>
          <PlayPause
            isPlaying={isPlaying}
        ...
        <img 
          alt="song_img" 
          src={song?.images?.coverart || song?.attributes?.artwork?.url || 'https://via.placeholder.com/300x300?text=No+Image'} 
